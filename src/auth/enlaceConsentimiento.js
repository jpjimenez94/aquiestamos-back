...
export function crearEnlaceConsentimiento(appointmentId) {
  return crearCompacto(CODIGO.consentimiento, appointmentId, Date.now() + TTL_MS)
}
...
export function leerEnlaceConsentimiento(token) {
  ...
  const compacto = leerCompacto(token, CODIGO.consentimiento)
  if (compacto) return { tipo: TIPO, cita: compacto.uuid, vence: compacto.vence }
